...
#import "EasyCentralManager.h"
#import "EasyCallback.h"
#import "NSTimer+Add.h"

@interface EasyCentralManager (){
    //已经连接的设备
...
@implementation EasyCentralManager


#define currChannel [easySpeaker callbackOnCurrChannel]
...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSDictionary *options = [NSDictionary dictionaryWithObjectsAndKeys:
                                 ...
                                 @"EasyBluetoothRestore",CBCentralManagerOptionRestoreIdentifierKey,
                                 nil];
...
- (void)scanPeripherals {
    [centralManager scanForPeripheralsWithServices:[currChannel easyOptions].scanForPeripheralsWithServices options:[currChannel easyOptions].scanForPeripheralsWithOptions];
}

//连接Peripherals
- (void)connectToPeripheral:(CBPeripheral *)peripheral{
    [centralManager connectPeripheral:peripheral options:[currChannel easyOptions].connectPeripheralWithOptions];
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    //发送通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtCentralManagerDidUpdateState object:@{@"central":central}];
    
    switch (central.state) {
        case CBManagerStateUnknown:
            EasyLog(@">>>CBCentralManagerStateUnknown");
            break;
        case CBManagerStateResetting:
            EasyLog(@">>>CBCentralManagerStateResetting");
            break;
        case CBManagerStateUnsupported:
            EasyLog(@">>>CBCentralManagerStateUnsupported");
            break;
        case CBManagerStateUnauthorized:
            EasyLog(@">>>CBCentralManagerStateUnauthorized");
            break;
        case CBManagerStatePoweredOff:
            EasyLog(@">>>CBCentralManagerStatePoweredOff");
            break;
        case CBManagerStatePoweredOn:
            EasyLog(@">>>CBCentralManagerStatePoweredOn");
            [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtCentralManagerEnable object:@{@"central":central}];
            break;
        default:
            break;
    }
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    //日志
    //EasyLog(@"当扫描到设备:%@",peripheral.name);
    [self addDiscoverPeripheral:peripheral];
    
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidDiscoverPeripheral
                                                       object:@{
                                                                @"central":central,
    ...
    if ([currChannel filterOnDiscoverPeripherals]) {
        if ([currChannel filterOnDiscoverPeripherals](peripheral.name,advertisementData,RSSI)) {
            if ([currChannel blockOnDiscoverPeripherals]) {
                [[easySpeaker callbackOnCurrChannel] blockOnDiscoverPeripherals](central,peripheral,advertisementData,RSSI);
            }
        }
    }
    ...
    if (needConnectPeripheral) {
        if ([currChannel filterOnconnectToPeripherals](peripheral.name,advertisementData,RSSI)) {
            ...
            connectTimer = [NSTimer safe_scheduledTimerWithTimeInterval:5.0f block:^{
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidConnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral}];
    
    //设置委托
    [peripheral setDelegate:self];
    
    //EasyLog(@">>>连接到名称为（%@）的设备-成功",peripheral.name);
    ...
    if (needDiscoverServices) {
        [peripheral discoverServices:[currChannel easyOptions].discoverWithServices];
    }
}
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidFailToConnectPeripheral
                                                       object:@{
                                                                @"central":central,
                                                                @"peripheral":peripheral,
                                                                @"error":error?error:@""
    ...
    //    EasyLog(@">>>连接到名称为（%@）的设备-失败,原因:%@",[peripheral name],[error localizedDescription]);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidDisconnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral,@"error":error?error:@""}];
    
    //    EasyLog(@">>>外设连接断开连接 %@: %@\n", [peripheral name], [error localizedDescription]);
    if (error){
        EasyLog(@">>> didDisconnectPeripheral for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidDiscoverServices
                                                       object:@{@"peripheral":peripheral,@"error":error?error:@""}];
    if (error) {
        EasyLog(@">>>didDiscoverServices for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    //回叫block
    ...
    if (needDiscoverCharacteristics) {
        for (CBService *service in peripheral.services) {
            [peripheral discoverCharacteristics:[currChannel easyOptions].discoverWithCharacteristics forService:service];
        }
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidDiscoverCharacteristicsForService
                                                       object:@{@"peripheral":peripheral,@"service":service,@"error":error?error:@""}];
    if (error) {
        EasyLog(@"error didDiscoverCharacteristicsForService for %@ with error: %@", service.UUID, [error localizedDescription]);
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidUpdateValueForCharacteristic
                                                       object:@{
                                                                @"peripheral":peripheral,
    ...
    if (error) {
        EasyLog(@"error didUpdateValueForCharacteristic %@ with error: %@", characteristic.UUID, [error localizedDescription]);
    }
    
    //查找字段订阅
    if ([easySpeaker notifyCallback:characteristic]) {
        [easySpeaker notifyCallback:characteristic](peripheral,characteristic,error);
        return;
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (error) {
        EasyLog(@"error Discovered DescriptorsForCharacteristic for %@ with error: %@", characteristic.UUID, [error localizedDescription]);
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    if (error) {
        EasyLog(@"error didUpdateValueForDescriptor  for %@ with error: %@", descriptor.UUID, [error localizedDescription]);
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidWriteValueForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:EasyNotificationAtDidUpdateNotificationStateForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
    
    EasyLog(@">>>didUpdateNotificationStateForCharacteristic");
    EasyLog(@">>>uuid:%@,isNotifying:%@",characteristic.UUID,characteristic.isNotifying?@"isNotifying":@"Notifying");
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(nullable NSError *)error {
    [[NSNotificationCenter defaultCenter] postNotificationName:EasyNotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":peripheral.RSSI,@"error":error?error:@""}];
    
    EasyLog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",peripheral.RSSI);
...
- (void)peripheral:(CBPeripheral *)peripheral didReadRSSI:(NSNumber *)RSSI error:(NSError *)error {
    [[NSNotificationCenter defaultCenter] postNotificationName:EasyNotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":RSSI?RSSI:@100,@"error":error?error:@""}];
    
    EasyLog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",RSSI);
    if ([currChannel blockOnDidReadRSSI]) {
